...
| 2026-07-29 | P4-T6 matchmaking ممالك | COMMIT_HASH | data/matchmaking.json جديد (استراتيجية least_fill + round_robin احتياطية + guardrails: hard_cap_ratio=1.0 وprefer_below_ratio=0.9)؛ migration 0008 (players.kingdom_id + جدول kingdom_assignments بسجل التعيين/الاستراتيجية/نسبة الامتلاء)؛ sim/matchmaking.ts جديد يقرأ JSON فقط (fillRatio + chooseKingdom حتمي بكسر تعادل: الأقل عدداً ثم الأبجدي)؛ city/init: عند توفر ممالك إطلاق مفتوحة يُعيَّن اللاعب للأقل امتلاءً بدل افتراض مملكة البيئة + بوابة سعة لكل مملكة + تسجيل التعيين في D1 + kingdom وmatchmaking{strategy,fillRatio,reason} في الاستجابة؛ GET /v1/city يكسب kingdom؛ GET /v1/matchmaking/status عام (إشغال كل مملكة + أحدث 10 تعيينات)؛ matchmaking_offline_test.mjs جديد: 30 فحصاً — كل الاختبارات الأوفلاين الـ 12 ناجحة بدون كسر |
---
