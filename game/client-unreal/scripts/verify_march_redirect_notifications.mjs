...
const required = [
  ...
  ['shard', /if \(now >= march\.etaMs\) \{\s*const tickInDay = [\s\S]*?;\s*await this\.resolveMarchArrival\(march, now, tickInDay\);\s*throw new Error\("march_already_arrived"\);/s, 'حسم الوصول قبل قبول التحويل المتأخر'],
  ...
  ['apiSource', /\[WeakThis\]\(const FString& Err\)/, 'معالج خطأ HTTP للتحويل بمؤشر ضعيف'],
